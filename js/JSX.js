...
// If you turn over is more the $50,000,000 then contact me http://creative-scripts.com/contact                               //
...
// Written July 23 2017 Version 1                                                                                                     //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 *         Allow for easy replacement i.e. variable insertions and for forcing eval.
...
Jsx.prototype.evalScript = function() {
    var arg, i, key, replaceThis, withThis, args, callback, csInterface, forceEval, replacements, jsxScript;
 ...
 *         
 *         The jsx.evalFiles method is used for calling jsx scripts
 ...
 *                             [Optional DEFAULT false]
 *
 * @param  {Object} forceEvalScript
 *                             If no replacements are needed then the jsx script can be executed by using the $.evalFile method
 ...
 *                             To force the forceEvalScript to force then either send a fake replacement like {YouWillNotFindMe: 'ha ha'} or {}
 *                             or if using an object set the forceEvalScript key to false
 *                             [Optional DEFAULT true when no replacements made]
 * 
 * Note 1) The order of the parameters is irrelevant
 * Note 2) One can pass the arguments as an object if desired
 *         jsx.evalScript(myCallBackFunction, 'alert("__myMessage__");', true);
...
Jsx.prototype.evalFile = function() {
    var __dirname, arg, args, arrayOfArrays, callback, csInterface, fileName, fileNameScript, forceEval, forceEvalScript,
        fs, i, jsxFolder, jsxScript, key, newLine, path, relReg, replacements, replaceThis, success, withThis;
    ...
    if (args.length === 1 && (arg = args[0]) instanceof Object) {
        ...
        replacements = arg.replacements || arg.replace;
        forceEval = arg.eval || arg.forceEval || arg.forceeval;
        forceEvalScript = arg.evalScript || arg.forceEvalScript || arg.forceevalscript || arg.evalscript;
    } else {
        for (i = 0; i < 5; i++) {
            arg = args[i];
            if (arg === undefined) {
    ...
    if (!replacements && (forceEvalScript === undefined)) { forceEvalScript = true; }
    // jerkproof
    if (forceEvalScript) { forceEval = false; }
    ...
    if (/^\/|[a-zA-Z]+:/.test(jsxScript)) {
        ...
        jsxScript = path.join(__dirname, jsxScript);
        jsxScript = path.normalize(jsxScript);
    } else {
        jsxScript = path.join(jsxFolder, jsxScript);
        jsxScript = path.normalize(jsxScript);
    }
    ...
    if (forceEvalScript) {
        ...
        // Check that the path exist, should change this to not asynchronous at some point
        try {
        ...
        jsxScript = fileNameScript.replace(/__fileName__/, jsxScript).replace(/__basename__/, path.win32.basename(jsxScript)) +
            '$.evalFile(new File("' + jsxScript + '", 10000000000)) + "";';
        csInterface.evalScript(jsxScript, callback);
        return true;
    }
